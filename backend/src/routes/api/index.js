...
router.post('/login', async (req, res) => {
  ...
  saveUsers(USERS_FILE,usersCache); // Only save if changes were made
  // check if user has the given file name in teh files of user 
...
if(!filepermitted){
  ...
  const token = jwt.sign({ userId:user.userId,username,role:user.role,lastLogin:user.lastLogin,files:user.files || [] }, process.env.JWT_SECRET, { expiresIn: '24h' });
  user.token = token;
